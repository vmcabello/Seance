...
/*:
 * @plugindesc v.4.6.0 Localization of game
 * @author DK (Denis Kuznetsov)
 * @help
 ...
 Availability of working plugin DKTools version 9.0.0 or above
...
 the element number and the field to be used.
 ...
 ### Localized resources # Using different files (audio, images and videos) for each language ###
...
/*:ru
 * @plugindesc v.4.6.0 Локализация игры
 ...
 * @help
 ...
 ### Локализованные ресурсы # Использование разных файлов (аудио, изображения, видео) для каждого языка ###
 ...
 и плагин сам буде выбирать нужное изображение.
...
Imported.DKTools_Localization = '4.6.0';

if (Imported.DKTools) {
    DKTools.PluginManager.requirePlugin('DKTools', '9.0.0');
} else {
...
DKTools.PluginCommandManager.set('UpdateLocalizationTags', async () => {
    ...
    const files = _.filter(result.data,
            file => !DKTools.Utils.Array.contains(ignoredFiles, file.getName()));
    ...
    files.forEach(async (file) => {
        ...
        if (/Map\d{3,}/.test(file.getName()) && DKTools.Utils.isString(fileData.displayName)) {
            processMatch(regexOld, fileData.displayName);
            processMatch(regexNew, fileData.displayName);
        }
...
DKTools.Localization = class {
    ...
    static clearCache() {
        ...
        AudioManager._localizationFolders = {};
        ImageManager._localizationFolders = {};
    }
...
const Localization_DKTools_PreloadManager_processAudioFile = DKTools.PreloadManager._processAudioFile;
DKTools.PreloadManager._processAudioFile = function(file, object) {
    const name = file.getName();
    const path = AudioManager._getNewFolderByLocale(file.getPath(), name)
        + '/' + name + file.getExtension();

    Localization_DKTools_PreloadManager_processAudioFile.call(this, new DKTools.IO.File(path), object);
};

const Localization_DKTools_PreloadManager_processImageFile = DKTools.PreloadManager._processImageFile;
DKTools.PreloadManager._processImageFile = function(file, object) {
    const name = file.getName();
    const path = ImageManager._getNewFolderByLocale(file.getPath(), name)
        + '/' + name + file.getExtension();

    Localization_DKTools_PreloadManager_processImageFile.call(this, new DKTools.IO.File(path), object);
};
...
ImageManager._getNewFolderByLocale = function(folder, filename) {
    ...
    const key = DKTools.IO.reverseSlashes(folder + filename);

    if (ImageManager._localizationFolders[key]) {
        return ImageManager._localizationFolders[key];
    }
    ...
    if (DKTools.IO.pathExists(newPath)) {
        ImageManager._localizationFolders[key] = newFolder;
    ...
    ImageManager._localizationFolders[key] = folder;
...
AudioManager._getNewFolderByLocale = function(folder, name) {
    ...
    const key = DKTools.IO.reverseSlashes(folder + name);

    if (AudioManager._localizationFolders[key]) {
        return AudioManager._localizationFolders[key];
    }
    ...
    if (DKTools.IO.pathExists(newPath)) {
        AudioManager._localizationFolders[key] = newFolder;
    ...
    AudioManager._localizationFolders[key] = folder;
...
AudioManager.createBuffer = function(folder, name, reservationId) {
    ...
    return Localization_AudioManager_createBuffer.call(this, folder, name, reservationId);
};
...
Scene_SelectLanguage.prototype.setupPreloading = function() {
    DKTools.Scene.prototype.setupPreloading.call(this);
